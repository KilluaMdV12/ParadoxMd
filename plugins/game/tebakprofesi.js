...
const pluginConfig = {
    ...
    alias: ['tp', 'guessjob'],
    category: 'game',
    description: 'Tebak profesi dari deskripsi yang diberikan',
    usage: '.tebakprofesi',
    example: '.tebakprofesi',
    isOwner: false,
    isPremium: false,
    isGroup: false,
    isPrivate: false,
...
async function handler(m, { sock }) {
    ...
    if (hasActiveSession(chatId)) {
        ...
        if (session && session.gameType === 'tebakprofesi') {
    ...
    const question = getRandomItem('tebakprofesi.json');
    if (!question) {
        await m.reply('❌ *ᴅᴀᴛᴀ ᴛɪᴅᴀᴋ ᴛᴇʀsᴇᴅɪᴀ*\n\n> Data game tidak tersedia!');
        return;
    }

    let text = `👔 *ᴛᴇʙᴀᴋ ᴘʀᴏꜰᴇsɪ*\n\n`;
    text += `\`\`\`${question.soal}\`\`\`\n\n`;
    text += `> 💡 Hint: *${getHint(question.jawaban, 2)}*\n`;
    text += `> ⏱️ Waktu: *60 detik*\n`;
    text += `> 🎁 Hadiah: *+${GAME_REWARD.limit} Limit, +${GAME_REWARD.koin} Bal, +${GAME_REWARD.exp} EXP*\n\n`;
    text += `_Reply pesan ini dengan jawabanmu atau ketik "nyerah"_`;

    const sentMsg = await sock.sendMessage(chatId, { text, contextInfo: getGameContextInfo('👔 TEBAK PROFESI', 'Tebak profesi dari deskripsi!') }, { quoted: m });
    createSession(chatId, 'tebakprofesi', question, sentMsg.key, 60000);

    setSessionTimer(chatId, async () => {
        let timeoutText = `⏱️ *ᴡᴀᴋᴛᴜ ʜᴀʙɪs!*\n\n> Jawaban: *${question.jawaban}*\n\n_Tidak ada yang berhasil menjawab_`;
...
async function answerHandler(m, sock) {
    const chatId = m.chat;
    const session = getSession(chatId);
    if (!session || session.gameType !== 'tebakprofesi') return false;
