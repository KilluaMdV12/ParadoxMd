...
const pluginConfig = {
    ...
    category: 'game',
    description: 'Tebak negara dari fakta menarik',
    usage: '.tebaknegara',
    example: '.tebaknegara',
    isOwner: false,
    isPremium: false,
...
async function handler(m, { sock }) {
    ...
    if (hasActiveSession(chatId)) {
        ...
        if (session && session.gameType === 'tebaknegara') {
    ...
    const question = getRandomItem('tebaknegara.json');
    if (!question) {
        await m.reply('❌ *ᴅᴀᴛᴀ ᴛɪᴅᴀᴋ ᴛᴇʀsᴇᴅɪᴀ*\n\n> Data game tidak tersedia!');
        return;
    }

    let text = `🌍 *ᴛᴇʙᴀᴋ ɴᴇɢᴀʀᴀ*\n\n\`\`\`${question.soal}\`\`\`\n\n> 💡 Hint: *${getHint(question.jawaban, 2)}*\n> ⏱️ Waktu: *60 detik*\n> 🎁 Hadiah: *+${GAME_REWARD.limit} Limit, +${GAME_REWARD.koin} Bal, +${GAME_REWARD.exp} EXP*\n\n_Reply pesan ini dengan jawabanmu atau ketik "nyerah"_`;

    const sentMsg = await sock.sendMessage(chatId, { text, contextInfo: getGameContextInfo('🌍 TEBAK NEGARA', 'Tebak negara dari fakta!') }, { quoted: m });
    createSession(chatId, 'tebaknegara', question, sentMsg.key, 60000);
...
async function answerHandler(m, sock) {
    const chatId = m.chat;
    const session = getSession(chatId);
    if (!session || session.gameType !== 'tebaknegara') return false;
