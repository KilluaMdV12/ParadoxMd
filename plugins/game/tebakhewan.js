...
const pluginConfig = {
    ...
    category: 'game',
    description: 'Tebak hewan dari ciri-ciri yang diberikan',
    usage: '.tebakhewan',
    example: '.tebakhewan',
    isOwner: false,
    isPremium: false,
...
async function handler(m, { sock }) {
    ...
    if (hasActiveSession(chatId)) {
        ...
        if (session && session.gameType === 'tebakhewan') {
    ...
    const question = getRandomItem('tebakhewan.json');
    if (!question) {
        await m.reply('❌ *ᴅᴀᴛᴀ ᴛɪᴅᴀᴋ ᴛᴇʀsᴇᴅɪᴀ*\n\n> Data game tidak tersedia!');
        return;
    }

    let text = `🐾 *ᴛᴇʙᴀᴋ ʜᴇᴡᴀɴ*\n\n`;
    text += `\`\`\`${question.soal}\`\`\`\n\n`;
    ...
    const sentMsg = await sock.sendMessage(chatId, { text, contextInfo: getGameContextInfo('🐾 TEBAK HEWAN', 'Tebak hewan dari ciri-ciri!') }, { quoted: m });

    createSession(chatId, 'tebakhewan', question, sentMsg.key, 60000);
...
async function answerHandler(m, sock) {
    ...
    if (!session || session.gameType !== 'tebakhewan') return false;
