...
const pluginConfig = {
    ...
    category: 'game',
    description: 'Pecahkan teka-teki logika yang menantang',
    usage: '.riddle',
    example: '.riddle',
    isOwner: false,
    isPremium: false,
...
async function handler(m, { sock }) {
    ...
    if (hasActiveSession(chatId)) {
        ...
        if (session && session.gameType === 'riddle') {
    ...
    const question = getRandomItem('riddle.json');
    if (!question) {
    ...
    const sentMsg = await sock.sendMessage(chatId, { text, contextInfo: getGameContextInfo('🧩 RIDDLE', 'Pecahkan teka-teki logika!') }, { quoted: m });
    createSession(chatId, 'riddle', question, sentMsg.key, 90000);
...
async function answerHandler(m, sock) {
    const chatId = m.chat;
    const session = getSession(chatId);
    if (!session || session.gameType !== 'riddle') return false;
