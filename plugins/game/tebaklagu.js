...
async function handler(m, { sock }) {
    ...
    if (hasActiveSession(chatId)) {
        ...
        if (session && session.gameType === 'tebaklagu') {
    ...
    const sentMsg = await sock.sendMessage(chatId, { text, contextInfo: getGameContextInfo('🎵 TEBAK LAGU', 'Tebak judul lagu dari lirik!') }, { quoted: m });
    createSession(chatId, 'tebaklagu', question, sentMsg.key, 60000);
...
async function answerHandler(m, sock) {
    const chatId = m.chat;
    const session = getSession(chatId);
    if (!session || session.gameType !== 'tebaklagu') return false;
