...
async function handler(m, { sock }) {
    ...
    if (hasActiveSession(chatId)) {
        ...
        if (session && session.gameType === 'kataacak') {
            const remaining = getRemainingTime(chatId);
            let text = `⚠️ *ᴍᴀsɪʜ ᴀᴅᴀ ɢᴀᴍᴇ*\n\n`;
            text += `> 🔀 Huruf: *${session.question.soal}*\n`;
    ...
    const question = getRandomItem('kataacak.json');
    if (!question) {
    ...
    let text = `🔀 *ᴋᴀᴛᴀ ᴀᴄᴀᴋ*\n\n`;
    text += `> Susun huruf berikut menjadi kata yang benar:\n\n`;
    text += `\`\`\`${question.soal}\`\`\`\n\n`;
    text += `> 💡 Hint: *${getHint(question.jawaban, 1)}*\n`;
    ...
    const sentMsg = await sock.sendMessage(chatId, { text, contextInfo: getGameContextInfo('🔀 KATA ACAK', 'Susun huruf jadi kata!') }, { quoted: m });
    createSession(chatId, 'kataacak', question, sentMsg.key, 45000);
...
async function answerHandler(m, sock) {
    const chatId = m.chat;
    const session = getSession(chatId);
    if (!session || session.gameType !== 'kataacak') return false;
