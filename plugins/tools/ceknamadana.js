...
async function handler(m, { sock }) {
    ...
    const Laww = text?.replace?.("62", "08")
    try {
        const axios = require("axios")
        let { data } = await axios.get('https://api.pitucode.com/cek-name-e-wallet-id-v2?bank=DANA&accountNumber='+Laww, {
      headers: {
  "x-api-key": "7C0dEefbfc1"
}
  })
        await m.reply(`🌿 *BERHASIL*\n\n- Nama dana dari nomor \`${Laww}\` adalah *${data.data.customer_name}*`)
    } catch (error) {
