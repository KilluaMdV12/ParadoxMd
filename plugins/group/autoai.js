...
const pluginConfig = {
    ...
    usage: '.autoai on/off --zenosmode=<character> --type=<text|voice>',
    example: '.autoai on --zenosmode=furina --type=voice',
    isOwner: false,
    isPremium: false,
    isGroup: true,
    isPrivate: false,
...
async function handler(m) {
    ...
    const modeMatch = fullArgs.match(/--zenosmode=(\w+)/i)
    ...
    if (!mode || !['on', 'off'].includes(mode)) {
        ...
        txt += `*Penggunaan:*\n`
        txt += `> .autoai on --zenosmode=<karakter> --type=<text|voice>\n`
        txt += `> .autoai off\n\n`
        ...
        txt += `*Contoh:*\n`
        txt += `> .autoai on --zenosmode=furina --type=text\n`
        txt += `> .autoai on --zenosmode=kobo --type=voice`
        return m.reply(txt)
    }
