...
const gameThumbPath = path.join(assetsPath, 'zenos-games.jpg')
const rpgThumbPath = path.join(assetsPath, 'zenos-rpg.jpg')
const winnerThumbPath = path.join(assetsPath, 'zenos-winner.jpg')
...
function getGameContextInfo(title = '🎮 ZENOS GAMES', body = 'Have fun playing!') {
...
function getRpgContextInfo(title = '⚔️ ZENOS RPG', body = 'Adventure awaits!') {
    const saluranId = config.saluran?.id || '120363407633768956@newsletter'
    const saluranName = config.saluran?.name || config.bot?.name || 'Zenos-AI'
...
function createFakeQuoted(botName = 'Zenos-AI', verified = true) {
    return {
        key: {
            fromMe: false,
