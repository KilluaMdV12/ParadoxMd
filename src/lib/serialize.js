...
function createContextInfo(jid, text, title = '', body = '', thumbnail = null) {
    ...
    if (jid && text) {
        ...
        contextInfo.stanzaId = 'ZENOSAI' + Date.now();
    }

    if (title || body || thumbnail) {
        contextInfo.externalAdReply = {
            showAdAttribution: true,
            title: title || config.bot?.name || 'Zenos-AI',
            body: body || '',
            mediaType: 1,
...
async function serialize(sock, msg, store = {}) {
    ...
    m.botName = sock.user?.name || config.bot?.name || 'Zenos-AI';
    m.messageId = m.id;
    m.chatId = m.chat;
    m.senderId = m.sender;
    ...
    m.reply = async (text, options = {}) => {
        ...
        if (replyVariant >= 2 && replyVariant <= 6) {
            try {
                const thumbPath = replyVariant <= 4 ? './assets/images/zenos2.jpg' : './assets/images/zenos.jpg';
                ...
                if (replyVariant === 2) {
                    contextInfo = {
                        ...contextInfo,
                        externalAdReply: {
                            title: config.bot?.name || 'Zenos-AI',
                    ...
                    contextInfo = {
                        ...
                        forwardedNewsletterMessageInfo: {
                            newsletterName: config.saluran?.name || 'Zenos-AI',
                            newsletterJid: config.saluran?.id ? `${config.saluran.id}@newsletter` : '120363407633768956@newslette',
                    ...
                    contextInfo = {
                        ...
                        forwardedNewsletterMessageInfo: {
                            newsletterName: config.saluran?.name || 'Zenos-AI',
                            newsletterJid: config.saluran?.id ? config.saluran.id : '120363407633768956@newslette',
                        },
                    };
        ...
        if (replyVariant === 4) {
            const saluranId = config.saluran?.id || '120363407633768956@newslette';
            ...
            let troliThumbnail = getCachedThumb('./assets/images/zenos.jpg');
            ...
            quotedMsg = {
                ...
                message: {
                    orderMessage: {
                        orderId: '1337',
                        thumbnail: await getCachedSharpThumb('./assets/images/zenos2.jpg', 300, 300),
                        itemCount: `BOT READY`,
                        status: 'INQUIRY',
                        surface: 'CATALOG',
                        message: `${config.bot?.name || 'Zenos-AI'}`,
                        ...
                        contextInfo: {
                            ...
                            forwardedNewsletterMessageInfo: {
                                newsletterJid: config.saluran?.id || '120363407633768956@newsletter',
            ...
            return sock.sendMessage(m.chat, {
                ...
                jpegThumbnail: await getCachedSharpThumb(join(process.cwd(), 'assets/images/zenos2.jpg'), 300, 300),
                caption: text,
                ...defaultOptions,
                ...options
            }, {
    ...
    m.replyWithPreview = async (text, preview, options = {}) => {
        return sock.sendMessage(m.chat, {
            ...
            contextInfo: {
                externalAdReply: {
                    showAdAttribution: true,
                    title: preview.title || config.bot?.name || 'Zenos-AI',
                    body: preview.body || '',
                    mediaType: 1,
                    renderLargerThumbnail: true,
